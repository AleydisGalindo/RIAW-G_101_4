...
def _build_url(row):
    url = ""
    try:
        url = row["url"]["urls"][0]["url"]  # tweet URL ["entities"]
    except:
        try:
            url = row["extended_entities"]["media"][0]["url"] #row["retweeted_status"]["extended_tweet"]["entities"]["media"][0]["url"]  # Retweeted
        except:
            url = f"https://twitter.com/{row['screen_name']}/status/{row['user_id_str']}"
    return url
...
def _build_terms(row):

    filtered_line = row.lower() ## Transform to lowercase
...
def _row_to_doc_dict(row: pd.Series):
    _corpus[row['Id']] = Document(row['Id'], row['Tweet'][0:100], row['Tweet'], _build_terms(row['Tweet']), row['Date'], row['Likes'],
                                  row['Retweets'],
